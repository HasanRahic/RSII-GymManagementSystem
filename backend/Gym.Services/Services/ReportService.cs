...
public class ReportService : IReportService
{
    ...
    public async Task<DashboardStatsDto> GetDashboardStatsAsync(int? gymId)
    {
        ...
        var revenueQ = SuccessfulPaymentsInRange(monthStart, null);
        ...
        if (gymId.HasValue)
        {
            membersQ = membersQ.Where(u => u.Memberships.Any(m => m.GymId == gymId.Value));
            membershipsQ = membershipsQ.Where(m => m.GymId == gymId.Value);
            checkInsQ = checkInsQ.Where(c => c.GymId == gymId.Value);
            revenueQ = revenueQ.Where(p =>
                (p.UserMembership != null && p.UserMembership.GymId == gymId.Value) ||
                (p.SessionReservation != null && p.SessionReservation.TrainingSession.GymId == gymId.Value) ||
                (p.ShopOrder != null && p.ShopOrder.GymId == gymId.Value));
    ...
    public async Task<IEnumerable<CheckInDto>> GetCheckInReportAsync(int? gymId, DateTime from, DateTime to)
    {
        ...
        {
            var dur = c.CheckOutTime.HasValue
    ...
    {
        var query = SuccessfulPaymentsInRange(from, to);
    ...
    private IQueryable<Payment> SuccessfulPaymentsInRange(DateTime fromInclusive, DateTime? toInclusive)
    {
        var query = _context.Payments
            .AsNoTracking()
            .Where(p => p.Status == PaymentStatus.Succeeded && p.CreatedAt >= fromInclusive);
