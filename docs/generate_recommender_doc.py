...
def build_pdf(output_path: Path, code_capture: Path, app_capture: Path) -> None:
    ...
    y -= 0.9 * cm
    pdf.setFont("Helvetica", 11)
    pdf.drawString(2.4 * cm, y, "flutter-mobile/lib/services/api_services.dart -> getRecommendedGyms")
    y -= 0.6 * cm
    pdf.drawString(2.4 * cm, y, "flutter-mobile/lib/screens/home_screen.dart -> prikaz preporuka i trenera")
